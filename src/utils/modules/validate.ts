...
import zh_lang from '@/i18n/languages/zh-cn';
import en_lang from '@/i18n/languages/en';
import { i18n } from '@/i18n';
import cache from '@/utils/modules/cache';

const TYPELIST: { [index: string]: string } = cache.getLanguage() == 'en' ? en_lang.validateType : zh_lang.validateType;

const DEFAULTMSG: { [index: string]: string } = cache.getLanguage() == 'en' ? en_lang.validateTip : zh_lang.validateTip;
...
function setRequiredStyle(dom: Element) {
  setTimeout(() => {
    ...
    span.innerText = i18n.global.t('validate.required');
