...
<template>
  ...
  <el-form-item v-bind="$props" v-else-if="type == 'select'" :style="style">
    <el-select v-model="fatherProps.modelValue[prop]" style="width: 100%" v-bind="$attrs" clearable>
      <el-option v-if="option && option.length" v-for="item in option" :key="item[optionValue]" :label="item[optionLabel]" :value="item[optionValue]"> </el-option>
      <slot></slot>
    </el-select>
  </el-form-item>
  <el-form-item v-bind="$props" v-else-if="type == 'selects'" :style="style">
    <el-select v-model="fatherProps.modelValue[prop]" style="width: 100%" v-bind="$attrs" multiple clearable collapse-tags>
      <el-option v-if="option && option.length" v-for="item in option" :key="item[optionValue]" :label="item[optionLabel]" :value="item[optionValue]"> </el-option>
      <slot></slot>
    </el-select>
  </el-form-item>
  ...
  <el-form-item v-bind="$props" v-else-if="type == 'checkbox'" :style="style">
    <el-checkbox-group v-model="fatherProps.modelValue[prop]" v-bind="$attrs">
      <el-checkbox v-for="item in option" :key="item[optionValue]" :label="item[optionLabel]" :name="item[optionValue]" />
    </el-checkbox-group>
  </el-form-item>
  ...
  <el-form-item v-bind="$props" v-else="type == 'default'" :style="style">
    <slot></slot>
  </el-form-item>
</template>
